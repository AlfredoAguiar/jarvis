...
def initialize_components():
    ...
    if EMBEDDING_MODEL_NAME == "openai":
        ...
        retriever = RAGRetriever(vector_db_path="chroma-openai", embedding_model_name=EMBEDDING_MODEL_NAME, api_key=api_key)
...
def query():
    ...
    llm_follow_up_questions = llm_model.generate_follow_up_questions(context=enhanced_context_text, question=query_text)
    sources_html = "<br>".join(sources)
    response_text = f"{llm_response}<br><br>Sources:<br>{sources_html}<br><br>Perguntas de Acompanhamento:<br>{llm_follow_up_questions}"
